...
class PvfModel {
    constructor() {
        ...
        this.fileVersion = 0;
        this.pvfPath = '';
        this.baseOffset = 0; // where encrypted file data starts
        this.childrenCache = new Map(); // parent -> immediate children (lazy)
        this.rootChildren = null;
        this.encodingCache = new Map(); // key -> detected encoding used on last read/write
    }
    async open(filePath, progress) {
        this.pvfPath = filePath;
        const fd = await fs.open(filePath, 'r');
        let pos = 0;
        const readBuf = async (len) => {
        ...
        const fileTree = crypto_1.PvfCrypto.decrypt(fileTreeEnc, fileTreeLen, fileTreeChecksum);
        this.fileList.clear();
        const dv = new DataView(fileTree.buffer, fileTree.byteOffset, fileTree.byteLength);
        let tp = 0;
        for (let i = 0; i < fileCount; i++) {
            const fileNameChecksum = dv.getUint32(tp, true);
            tp += 4;
            const nameLen = dv.getUint32(tp, true);
            tp += 4;
            const nameBytes = fileTree.subarray(tp, tp + nameLen);
            tp += nameLen;
            const dataLen = dv.getInt32(tp, true);
            tp += 4;
            const checksum = dv.getUint32(tp, true);
            tp += 4;
            const offset = dv.getInt32(tp, true);
            tp += 4;
            const pf = new PvfFile(fileNameChecksum, nameBytes, dataLen, checksum, offset);
            this.fileList.set(pf.fileName, pf);
            if (progress && i % 512 === 0)
                progress(Math.floor((i / fileCount) * 50));
        }
    ...
    getStringFromTable(index) { return this.strtable?.get(index); }
    getFileByKey(key) { return this.fileList.get(key); }
    async loadFileData(f) { return await this.readAndDecrypt(f); }
    async save(filePath, progress) {
        ...
        for (let i = 0; i < files.length; i++) {
            const f = files[i];
            dv.setUint32(tp, f.fileNameChecksum, true);
            tp += 4;
            dv.setUint32(tp, f.fileNameLen, true);
            tp += 4;
            fileTree.set(f.fileNameBytes, tp);
            tp += f.fileNameLen;
            dv.setInt32(tp, f.dataLen, true);
            tp += 4;
            dv.setUint32(tp, f.checksum, true);
            tp += 4;
            dv.setInt32(tp, dataOffset, true);
            tp += 4;
            dataOffset += f.blockLength;
            if (progress && i % 1024 === 0)
                progress(Math.floor((i / (files.length * 2)) * 100));
        }
        const fileTreeChecksum = crypto_1.PvfCrypto.createBuffKey(fileTree, fileTreeLen, this.fileList.size >>> 0);
        const fileTreeEnc = crypto_1.PvfCrypto.encrypt(fileTree, fileTreeLen, fileTreeChecksum);
        // layout sizes
        ...
        for (const f of files)
            totalSize += f.blockLength;
        totalSize += ending.length;
        const out = Buffer.alloc(totalSize);
        let p = 0;
        out.writeInt32LE(this.guidLen, p);
        p += 4;
        this.guid.copy(out, p);
        p += this.guidLen;
        out.writeInt32LE(this.fileVersion, p);
        p += 4;
        out.writeInt32LE(fileTreeLen, p);
        p += 4;
        out.writeUInt32LE(fileTreeChecksum, p);
        p += 4;
        out.writeInt32LE(fileCount, p);
        p += 4;
        Buffer.from(fileTreeEnc).copy(out, p);
        p += fileTreeEnc.length;
        ...
        try {
            for (let i = 0; i < files.length; i++) {
                ...
                if (f.blockLength > 0) {
                    if (f.changed && f.data) {
                        const enc = crypto_1.PvfCrypto.encrypt(f.data, f.blockLength, f.checksum);
                        Buffer.from(enc).copy(out, p);
                        p += enc.length;
                    }
                    else if (srcFd) {
                        ...
                        const enc = crypto_1.PvfCrypto.encrypt(data, f.blockLength, f.checksum);
                        Buffer.from(enc).copy(out, p);
                        p += enc.length;
                    }
                }
                if (progress && i % 512 === 0)
                    progress(Math.floor(((fileCount + i) / (fileCount * 2)) * 100));
            }
        }
        finally {
            if (srcFd)
    ...
    getChildren(parent) {
        if (!parent) {
            ...
            const dirs = [...folders.keys()].map(k => ({ key: k, name: k, isFile: false }));
        ...
        if (this.childrenCache.has(parent))
            return this.childrenCache.get(parent);
    ...
    updateFileData(key, content) {
        ...
        if (f.isScriptFile) {
            ...
            if (text.charCodeAt(0) === 0xFEFF)
                text = text.slice(1);
            if (!this.strtable)
                this.strtable = new StringTable(this.encodingForKey('stringtable.bin'));
    ...
    decompileScript(f) {
        ...
        const sb = [];
        sb.push('#PVF_File');
        const getStr = (idx) => this.strtable?.get(idx) ?? `#${idx}`;
        const getStrLink = (id, nameIdx) => this.strview?.get(id, getStr(nameIdx)) ?? '';
        let i = 0;
        while (i < items.length) {
            ...
            if (t === 5 || (this.strtable && getStr(v).startsWith('['))) {
                ...
                while (i < items.length) {
                    const nt = items[i].t;
                    const nv = items[i].v;
                    if (nt === 5 || (this.strtable && getStr(nv).startsWith('[')))
                        break;
                    ...
                    while (i < items.length) {
                        ...
                        const asFloat = Number.isFinite(f32) && (Math.abs(kv) > 1000000 || Math.abs(f32 % 1) > 1e-6);
            ...
            if (t === 7) {
                ...
                const asFloat = Number.isFinite(f32) && (Math.abs(v) > 1000000 || Math.abs(f32 % 1) > 1e-6);
    ...
    async readAndDecrypt(f) {
        ...
        try {
            ...
            const dec = crypto_1.PvfCrypto.decrypt(new Uint8Array(buf.buffer, buf.byteOffset, buf.byteLength), f.blockLength, f.checksum);
            // zero padding
            for (let i = 0; i < f.blockLength - f.dataLen; i++)
                dec[f.dataLen + i] = 0;
            f.data = dec; // cache
            return dec;
        }
    ...
    renderStringTableText() {
        if (this.strtable)
            return this.strtable.dumpText();
    ...
    async ensureStringTableUpToDate() {
        if (!this.strtable)
            return;
        if (!this.strtable.isUpdated)
            return;
        const f = this.fileList.get('stringtable.bin');
        if (!f)
            return;
...
exports.PvfModel = PvfModel;
class PvfFile {
    constructor(fileNameChecksum, fileNameBytes, dataLen, checksum, offset) {
        this.changed = false;
    ...
    initFile(enc) {
        this.data = crypto_1.PvfCrypto.decrypt(enc, this.blockLength, this.checksum);
        // zero trailing padding
        for (let i = 0; i < this.blockLength - this.dataLen; i++)
            this.data[this.dataLen + i] = 0;
    }
    writeFileData(bytes) {
        ...
        this.checksum = crypto_1.PvfCrypto.createBuffKey(this.data, block, this.fileNameChecksum);
    }
}
...
class StringTable {
    ...
    get(idx) { return (idx >= 0 && idx < this.list.length) ? this.list[idx] : `#{${idx}}`; }
    getIndex(str) { return this.list.indexOf(str); }
    add(str) { const i = this.list.indexOf(str); if (i >= 0)
        return i; this.list.push(str); this.isUpdated = true; return this.list.length - 1; }
    dumpText() { return this.list.map((s, i) => `${i}\t${s}`).join('\n'); }
    parseFromText(text) {
...
class ScriptCompiler {
    ...
    compile(scriptText) {
        try {
            ...
            for (const lineRaw of lines) {
                ...
                const hdr = line.trim();
                if (!hdr || hdr.toLowerCase() === '#pvf_file' || hdr.toLowerCase() === '#pvf_file_add')
                    continue;
    ...
    compileItem(item) {
        const res = [];
        const st = this.model['strtable'];
        const getIdx = (s) => st ? (st.getIndex(s) >= 0 ? st.getIndex(s) : st.add(s)) : 0;
        const trim = (s) => s.trim();
        if (!item)
            return res;
        // Section [name]
        ...
        if (item.startsWith('{') && item.endsWith('}')) {
            ...
            if (vStr.startsWith('`') && vStr.endsWith('`')) {
                ...
                if (vStr.indexOf('.') >= 0) {
                    ...
                    buf.writeFloatLE(isFinite(f) ? f : 0, 0);
...
class StringView {
    ...
    async init(nStringLstBytes, model, encoding) {
        ...
        const get4 = (pos) => dv.getInt32(pos, true);
