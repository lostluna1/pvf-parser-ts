        ...
        vscode.commands.registerCommand('pvf.openNpk', async () => {
            ...
            try {
                ...
                for (const a of albums) {
                    const v = a.version || 0;
                    verStat.set(v, (verStat.get(v) || 0) + 1);
                }
...
// Simple webview preview for an Album's sprite frames
function showAlbumPreview(album: any) {
        ...
        // Prepare minimal JSON payload: frame sizes and rgba data (base64)
        const items: any[] = [];
        for (let i = 0; i < Math.min(album.sprites?.length || 0, 64); i++) {
                const s = album.sprites[i];
                if (!s) continue;
                const rgba = getSpriteRgba(album, i);
                if (!rgba) continue;
        ...
        <head>
            ...
            <style>
                ...
                .item { border: 1px solid #ccc; padding: 8px; border-radius: 6px; background: #fafafa; }
                .cap { font-size: 12px; color: #555; margin-top: 4px; }
                canvas { background: transparent; image-rendering: pixelated; }
            </style>
        </head>
        <body>
            <div>帧总数：${album?.sprites?.length || 0}；展示前 ${items.length} 帧</div>
            <div class="grid" id="grid"></div>
            <script>
                const items = ${payload};
                const grid = document.getElementById('grid');
                function drawRGBAOnCanvas(rgba, w, h, canvas) {
                    canvas.width = w; canvas.height = h;
                    const ctx = canvas.getContext('2d');
                    const imgData = ctx.createImageData(w, h);
                    // Our rgba from extension may be either BGRA or RGBA; here we assume RGBA as provided.
                ...
                for (const it of items) {
                    ...
                    cap.className = 'cap';
                    cap.textContent = '#' + it.index + '  ' + it.w + 'x' + it.h;
                    wrap.appendChild(canvas);
                    wrap.appendChild(cap);
                    grid.appendChild(wrap);
                    const rgba = Uint8ClampedArray.from(atob(it.data), c => c.charCodeAt(0));
                    drawRGBAOnCanvas(rgba, it.w, it.h, canvas);
                }
            </script>
        </body>
        </html>`;
