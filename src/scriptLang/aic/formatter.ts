...
class AicFormatter implements FormatterStrategy {
  ...
  async provideEdits(ctx: FormatContext): Promise<vscode.TextEdit[]> {
    ...
    const tags = await loadTags(this.extCtx, 'aic');
      ...
      for (const t of iterateBracketTags(trimmed)) {
        if (!t.isClose) {
          ...
          if (closable.has(lower) && t.matchStart === 0) {
            ...
            valueParentDepth = depth; // 非闭合标签值块
          }
        }
        break;
      }
...
export function registerAicFormatter(context: vscode.ExtensionContext) {
  registerFormatter(context, 'pvf-aic', new AicFormatter(context));
}
