export class PvfCrypto {
  private static rotl(x: number, n: number) {
    return ((x << n) | (x >>> (32 - n))) >>> 0;
  }
  ...
  static createBuffKey(buf: Uint8Array, trueLen: number, fileNameHash: number): number {
  ...
  static decrypt(source: Uint8Array, len: number, checksum: number): Uint8Array {
  ...
  static encrypt(source: Uint8Array, len: number, checksum: number): Uint8Array {
    const key = 2175242257 >>> 0;
